...
import {
  ...
  type CreateInvitationsBulkRequest,
  type InvitationResponse,
} from "@event-platform/validators";

export function invitationsApi(client: AxiosInstance) {
  return {
    async createBulk(eventId: string, payload: CreateInvitationsBulkRequest): Promise<InvitationResponse[]> {
      const body = createInvitationsBulkRequestSchema.parse(payload);
      const res = await client.post(`/api/events/${eventId}/invitations/bulk`, body);
